...
public class PRUAdvisersEvidence {

	static Map captureAem(File evidenceDir, String stem) {
		evidenceDir.mkdirs()
		File dest = new File(evidenceDir, stem + '-aem.png')
		String url = PRUAdvisersForm.currentUrl()
		try {
			PRUAdvisersForm.pause(0.4)
			PRUAdvisersForm.scrollToSearch()
			PRUAdvisersForm.pause(0.3)
			fullPagePng(dest)
			KeywordUtil.logInfo('PRUAdvisers shot: ' + dest.name + ' bytes=' + dest.length() + ' <- ' + url)
		} catch (Exception e) {
