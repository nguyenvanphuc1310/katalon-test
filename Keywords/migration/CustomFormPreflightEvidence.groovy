...
public class CustomFormPreflightEvidence {

	static Map captureAem(File evidenceDir, String stem) {
		evidenceDir.mkdirs()
		File dest = new File(evidenceDir, stem + '-aem.png')
		String url = CustomFormPreflightForm.currentUrl()
		try {
			CustomFormPreflightForm.pause(0.4)
			CustomFormPreflightForm.scrollToForm()
			CustomFormPreflightForm.pause(0.3)
			fullPagePng(dest)
			KeywordUtil.logInfo('Custom form preflight shot: ' + dest.name + ' bytes=' + dest.length() + ' <- ' + url)
		} catch (Exception e) {
