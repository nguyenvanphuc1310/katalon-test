...
public class PpcSpecialistsEvidence {

	static Map captureAem(File evidenceDir, String stem) {
		evidenceDir.mkdirs()
		File dest = new File(evidenceDir, stem + '-aem.png')
		String url = PpcSpecialistsForm.currentUrl()
		try {
			PpcSpecialistsForm.pause(0.3)
			PpcSpecialistsForm.dismissPopup()
			PpcSpecialistsForm.scrollToSearch()
			PpcSpecialistsForm.pause(0.3)
			fullPagePng(dest)
			KeywordUtil.logInfo('PPC Specialists shot: ' + dest.name + ' bytes=' + dest.length() + ' <- ' + url)
		} catch (Exception e) {
