...
| OS | macOS — the project was built against `/Applications/Katalon Studio.app` |
...
**1. Clear the compiled keywords before the first run** — Katalon runs whatever is in
`bin/keyword/`, not what is in `Keywords/`, so a checkout that carries someone else's build
runs their code:

```bash
rm -rf bin/keyword .cache/Keywords
```
...
| `Data Files/baselines/` | the crawled snapshots — tracked in git: they are input to `mode=recompare`, not output |
...
| File | What |
|---|---|
| `Reports/parity-report/index.html` | the report — run facts and one row per template; open a template to reach its pages, open a page to reach its failing texts |
